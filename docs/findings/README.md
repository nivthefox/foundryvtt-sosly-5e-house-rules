...
- [x] Verify · [ ] Repair · [F-11: Psionic spell subtitle integration does not match the current dnd5e spellbook](F-11-psionic-subtitle-render-not-idempotent.md)—Current dnd5e rows no longer match the subtitle callback's selectors, so cost annotations and power-limit filtering do not run.
