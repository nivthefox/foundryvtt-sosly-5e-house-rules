...
- [x] Verify · [x] Repair · [F-13: Encumbrance registers an inert item update hook](F-13-encumbrance-uses-wrong-update-path.md)—The registered callback's guard checks the wrong path; Foundry has already re-prepared the parent Actor before the hook runs, and its direct Actor method call would not recalculate encumbrance.
